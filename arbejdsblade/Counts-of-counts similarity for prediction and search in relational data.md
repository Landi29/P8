...
If we take an example of how the logistic evaluation function works on the author TET which was used earlier and where $\beta = (-14.7, 5.8)$ then if we look at (C) in the picture above then the buttom true values are evaluated to 1. At the middel  level we evaluate the first value $\{t : 3\}$ as such: 
$$
