...
def manhatten_distance(tet1, tet2):
    '''
    description: this function finds the manhatten distance by treationg the tet histograms as vectors 
                 and compreing the elements in the vectors
...
def knn(user, others, k=3):
    '''
    description: this is a simpel implementation of knn finding the k nearest neighbors and making predictions
    parameters: user is a user you want to recommend to, others are all other trees and k is the nomber of neighbors we want to compare with.
...
def pred(user, others):
    '''
    description: this function calculates a prediction on the users rating on movies others has seen that the user have not seen
...
def reasing_sims(sims):
    ...
    return: the return is a list tubles where the disance has been reasinged to be used at simularety values.
    '''
...
def userdatabase():
    '''
    description: this function construnts a dictionary of users with movies they have seen and the rating given,
                 to use as a moch database over the users
