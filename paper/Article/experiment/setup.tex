\subsection{Setup}
In this experiment we test the reliability of the TET comparesons againt the other the embedding, SIMGNN and a bruteforce methods. 
...
From the data a weighted graph was created, represented as en edgelist. Each edge is of the form (MovieId, UserId, Rating) where the rating acts as a weight. Each node in the network is either a User or a Movie represented by their Id with the addition of a U or M respectively, to differentiate them.

The data will be split in 10 folds to keep the common structure in machine learning of a 80\%/20\% split to training- and testsets \cite{Ricci2015}, but we would like to add a validation dataset and will therefor have the splits be 80\%/10\%/10\% or 8 folds/1 fold/1 fold. The splits will be node on edge level meaning that training a user will have 70\% of their rations, we will in the validation and test reconstruct or predict the ratings in the set.
